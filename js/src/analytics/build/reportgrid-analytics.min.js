...
n(/https?:\/\/([^\/]+)\//,document.referrer)===(document.location.host||document.location.hostname)?+c("reportgrid_already_unbounced")||(f=+c("reportgrid_bounce_time"),c("reportgrid_already_unbounced",1),g("bounce",{},{path:c("reportgrid_bounce_path"),count:-1,timestamp:f})):(c("reportgrid_already_unbounced",0),f=+new Date,c("reportgrid_bounce_path",j),c("reportgrid_bounce_time",f),g("bounce",{},{timestamp:f}));b('a, button, input[type="submit"]').live("click",function(){var a=this.nodeName.toLowerCase();
